...
namespace NostreetsExtensions.Helpers.SqlTranslator
{
    ...
    /// </summary>
    public class LinqProvider : QueryProvider
    {
        DbConnection connection;
        TextWriter log;

        public LinqProvider(DbConnection connection)
        {
        ...
        public object Execute(string commandText, string[] paramNames, object[] paramValues, Func<DbDataReader, object> map)
        {
            if (this.log != null)
            {
        ...
        class ProjectionBuilder : DbExpressionVisitor
        {
            ...
            private ProjectionBuilder(LinqProvider provider, ProjectionExpression projection, Expression providerAccess)
            {
            ...
            internal static LambdaExpression Build(LinqProvider provider, ProjectionExpression projection, Expression providerAccess)
            {
            ...
            protected override Expression VisitProjection(ProjectionExpression projection)
            {
                ...
                LambdaExpression projector = Build(this.provider, projection, this.providerAccess);
                LambdaExpression eRead = GetReader(projector, projection.Aggregator, true);
